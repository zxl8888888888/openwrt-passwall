...
start_dns() {
	...
	[ -n "$CHINADNS_NG" ] && {
		local chnlist_param gfwlist_param extra_mode
		...
		ln_start_bin "$(first_type chinadns-ng)" chinadns-ng "/dev/null" -l "${china_ng_listen_port}" ${china_ng_chn:+-c "${china_ng_chn}"} ${chnlist_param} ${china_ng_gfw:+-t "${china_ng_gfw}"} ${gfwlist_param:+-g "${gfwlist_param}"} -f
		echolog "  + 过滤服务：ChinaDNS-NG(:${china_ng_listen_port}${extra_mode}) + ${msg}：国内DNS：${china_ng_chn:-D114.114.114.114}，可信DNS：${china_ng_gfw:-D8.8.8.8}"
...
add_dnsmasq() {
	...
	if [ "${DNS_MODE}" = "nonuse" ]; then
		...
		if [ -n "${USE_CHNLIST}" ] && [ -n "${gfwlist}" ]; then
			unset USE_CHNLIST
		fi
	...
	if [ "${DNS_MODE}" != "nouse" ]; then
		...
		if [ -z "${CHINADNS_NG}" ] && [ -z "${USE_CHNLIST}" ] && [ "${IS_DEFAULT_DNS}" = "1" ]; then
			echolog "  - 不强制设置默认DNS"
			return
		else
			...
			[ -n "${USE_CHNLIST}" ] && [ -z "${returnhome}" ] && [ -n "${chnlist}" ] && servers="${TUN_DNS}"
